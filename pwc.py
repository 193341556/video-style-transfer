...
def pwcflow(strOne,strTwo,strOut):

    tenOne = torch.FloatTensor(numpy.ascontiguousarray(
        numpy.array(PIL.Image.open(strOne))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
    tenTwo = torch.FloatTensor(numpy.ascontiguousarray(
        numpy.array(PIL.Image.open(strTwo))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
