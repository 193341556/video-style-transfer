...
class TemporalConsistencyLoss(nn.Module):
    def forward(self, gen_current, gen_next, flow):
        ...
        xx = xx.unsqueeze(0).expand(B, -1, -1).float()
        yy = yy.unsqueeze(0).expand(B, -1, -1).float()
        flow_x = flow[:, 0, :, :]
        flow_y = flow[:, 1, :, :]
        ...
        warped_current = nn.functional.grid_sample(
            gen_current, grid,
            mode='bilinear',
            padding_mode='zeros',
            align_corners=False
        )
        return TEMPORAL_WEIGHT * 10000 * nn.L1Loss()(warped_current, gen_next)
...
def train():
    ...
    if BATCH_SIZE > 1:
        repeated_style_tensor_for_vgg = style_tensor.repeat(BATCH_SIZE, 1, 1, 1)
    ...
    style_input_for_adain = torch.mean(style_features_for_adain_map, dim=[2, 3]) # Global Average Pooling
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch_idx, batch in progress_bar:
            ...
            sliced_style_input_for_adain = style_input_for_adain[:current_batch_size]

            gen_current = transformer_net(current, style_input_for_adain=sliced_style_input_for_adain)
            gen_next = transformer_net(next_frame, style_input_for_adain=sliced_style_input_for_adain)
            ...
            style_loss = 0
            # When calculating style loss, ensure style_grams are sliced correctly for the current batch size
            for layer, feat in gen_features.items():
                style_loss += STYLE_WEIGHT * nn.MSELoss()(utils.gram(feat), style_grams[layer][:current.size(0)])
