...
class DeconvLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, output_padding, norm="adain", style_feature_dim=None):
        ...
        if norm == "instance":
            ...
            if style_feature_dim is None:
                raise ValueError("style_feature_dim must be provided for AdaIN normalization in DeconvLayer")
            self.adain = AdaIN()
            self.mlp_style_mean = nn.Linear(style_feature_dim, out_channels)
            self.mlp_style_std = nn.Linear(style_feature_dim, out_channels)
        elif norm != "None":
...
class TransformerNetwork(nn.Module):
    def __init__(self, style_feature_dim=512, norm_type="adain"):
        ...
        self.relu5 = nn.ReLU()
        self.final_conv = ConvLayer(32, 3, 9, 1, norm="None") # Final layer no norm

    def forward(self, x, style_input_for_adain=None):
        if self.norm_type == "adain" and style_input_for_adain is None:
            raise ValueError("style_input_for_adain must be provided when norm_type is adain")

        # ConvBlock
        x = self.relu1(self.conv1(x, style_input_for_adain=style_input_for_adain))
        x = self.relu2(self.conv2(x, style_input_for_adain=style_input_for_adain))
        x_conv_out = self.relu3(self.conv3(x, style_input_for_adain=style_input_for_adain))

        # ResidualBlock
        x_res_out = x_conv_out
        for res_layer in self.res_blocks:
            x_res_out = res_layer(x_res_out, style_input_for_adain=style_input_for_adain)
        
        # DeconvBlock
        x = self.relu4(self.deconv1(x_res_out, style_input_for_adain=style_input_for_adain))
        x = self.relu5(self.deconv2(x, style_input_for_adain=style_input_for_adain))
